...
import { eolConverter } from "../IGenerationOptions";
...
export class FileProcessor {
    ...
    public removeUnusedImports(): FileProcessor {
        ...
        const openBracketIndex =
            this.renderedFile.indexOf("{", importStartIndex) + 1;
        const closeBracketIndex = this.renderedFile.indexOf(
            "}",
            openBracketIndex
        );
        ...
        if (distinctImports.length === 0) {
            const importEndIndex =
                this.renderedFile.indexOf(";", closeBracketIndex) + 1;
            this.renderedFile = `${this.renderedFile.substring(
                0,
                importStartIndex
            )}${this.renderedFile.substring(importEndIndex).trim()}`;
            return this;
        }
        ...
        this.renderedFile = `${this.renderedFile.substring(
            0,
            openBracketIndex
        )}${distinctImports.join(",")}${this.renderedFile.substring(
            closeBracketIndex
        )}`;
    ...
    public prettierFormat(prettierOptions?: Prettier.Options): FileProcessor {
        ...
        this.renderedFile = Prettier.format(
            this.renderedFile,
